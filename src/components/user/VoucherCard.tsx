...
import { Award, Star, Gift, CheckCircle2, AlertCircle, Sparkles, Smartphone } from "lucide-react";
import {
  Voucher,
  VoucherStatus,
  effectiveStatus,
...
import { PresentVoucherModal } from "./PresentVoucherModal";
...
interface VoucherCardProps {
  ...
  onPresent?: (voucher: Voucher) => void;
  onStatusChange?: (updated: Voucher) => void;
}
...
export function VoucherCard({ voucher: initialVoucher, onClaim, onPresent, onStatusChange }: VoucherCardProps) {
  ...
  const handleRedeemedInModal = (updated: Voucher) => {
    setVoucher(updated);
    if (onStatusChange) {
  ...
  return (
    <>
      ...
      >
        ...
        {/* Ticket Interactive Action Buttons */}
        <div className="flex flex-col gap-2.5 w-full mt-1">
          {status === "claimed" ? (
            <>
              ...
              <div className="w-full flex items-center justify-center gap-2 rounded-2xl bg-emerald-50 border border-emerald-200 py-2.5 text-xs font-bold text-emerald-700">
            ...
            <button
              onClick={handlePresentClick}
              className="w-full flex items-center justify-center gap-2 rounded-2xl bg-purple-50 border border-purple-200 py-3 text-xs sm:text-sm font-bold text-purple-700 hover:bg-purple-100/80 transition-colors"
            >
              <CheckCircle2 size={18} />
              <span>Redeemed &amp; Fulfilled 💕 (View Ticket)</span>
            </button>
          ) : claimable ? (
            <button
      ...
      {/* Interactive Present/Redeem Modal */}
      <PresentVoucherModal
        voucher={voucher}
        isOpen={isPresentModalOpen}
        onClose={() => setIsPresentModalOpen(false)}
        onRedeemed={handleRedeemedInModal}
      />
    </>
  );
}