...
export default function Page() {
  ...
  const [activeMonthsary, setActiveMonthsary] = useState<2 | 3 | 4 | 5 | 6>(6);
            ...
            <div className="mb-6 mt-4 flex items-center justify-center gap-1.5 bg-white/60 p-2 rounded-2xl border border-rose-100 shadow-md max-w-xl w-full backdrop-blur-sm">
              {[2, 3, 4, 5, 6].map((m) => (
                <button
                  key={m}
                  onClick={() => {
                    setActiveMonthsary(m as 2 | 3 | 4 | 5 | 6);
                    celebrate();
                  }}
                ...
                >
                  {m === 2 ? "2nd" : m === 3 ? "3rd" : m === 4 ? "4th" : m === 5 ? "5th" : m === 6 ? "6th" : ""} Monthsary
                </button>
              ))}
            </div>
