...
for(const path of ['public/folio.html','public/folio/index.html'])test(`${path}: Save project downloads every page as PDF`,async()=>{
 ...
 try{
  ...
  const a=w.api;a.state.drugs=a.state.drugs.slice(0,2);a.state.scope='selected';a.state.mode='free';
  a.state.free.text='My extra notes';
  const pages=a.projectPDFPages();
  assert.ok(pages.some(p=>p.drug===a.state.drugs[0]));assert.ok(pages.some(p=>p.drug===a.state.drugs[1]));
  assert.ok(pages.some(p=>p.raw.join(' ').includes('My extra notes')));
  assert.equal(a.state.scope,'selected');assert.equal(a.state.mode,'free');
